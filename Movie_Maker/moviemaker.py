...
for i in range(int((action[2]-action[1])/action[3])):
	...
	ax[0].plot(xaxis, jansky_variable_n0, '--', label='n=0', color='tab:red')
	ax[0].plot(xaxis, jansky_variable_n1, '--', label='n=1', color='tab:orange')
	ax[0].plot(xaxis, jansky_variable_n2, '--', label='n=2', color='tab:blue')
	ax[0].plot(xaxis, jansky_variable_total, '--', label = 'Total', color='tab:purple')

	ax[0].set_ylabel("Total Flux ({})".format(R'$J_y$'))
	ax[0].set_xscale('log')
	ax[0].set_yscale('log')
	# TODO: Fix for other #0s
	...
	p1 = ax[0].scatter(xaxis[i], jansky_variable_total[i], color='tab:purple')
	p2 = ax[0].scatter(xaxis[i], jansky_variable_n0[i], color='tab:red')
	p3 = ax[0].scatter(xaxis[i], jansky_variable_n1[i], color='tab:orange')
	p4 = ax[0].scatter(xaxis[i], jansky_variable_n2[i], color='tab:blue')
	...
	alpha0 =  ring_radii_n0_array * np.cos(theta)
	beta0 =  ring_radii_n0_array * np.sin(theta)
	alpha1 =  ring_radii_n1_array * np.cos(theta)
	beta1 =  ring_radii_n1_array * np.sin(theta)
	alpha2 =  ring_radii_n2_array * np.cos(theta)
	beta2 =  ring_radii_n2_array * np.sin(theta)


	ax[2] = plt.subplot(1,2,2)
	#im = ax[2].imshow(I0+I1+I2,vmax=np.max(I0+I1+I2)*1.2,origin="lower",cmap="afmhot",extent=[-lim0,lim0,-lim0,lim0])
	...
	ax[2].text(-9,8.5, label[action[0],0] + str(round(x_variable[i]/label[action[0],1], 2)) + ' ' + label[action[0],2], fontsize = 12, color="w")
...
np.save('R0' + ring_name, ring_radii_n0)
np.save('R1' + ring_name, ring_radii_n1)
np.save('R2' + ring_name, ring_radii_n2)
np.save('xaxis' + ring_name, xaxis)
